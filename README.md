...
`/help` · `/context` 看上下文状态 · `/portfolio` 看持仓 · `/plan` 看当前任务计划 · `/journal` 看日记 · `/memory` · `/sources` · `/cost` 看用量 · `exit` 退出（退出时自动整理记忆）。
...
footer 会显示工具错误、截断次数与上下文压缩情况。可用 `FIN_TOOL_TIMEOUT_SECONDS` 和
