...
用自写 Agent 循环 + OpenAI/Codex API 搭的本地金融 agent 工具。
...
```
它会引导写入本地 `.env`，默认使用 Codex/OpenAI 接口：`OPENAI_API_KEY` + `OPENAI_MODEL`。
也可以手动复制 `.env.example` 为 `.env` 后填写。

默认新会话，打开直接问。想接上次对话历史：启动时加 `-c`（`run.bat -c` 或 `python main.py -c`）。

**交互命令**（本地直接执行，不耗 token）：
`/help` · `/agent` 看能力画像 · `/doctor` 本地体检 · `/tools` 看工具目录 · `/trace` 看工具轨迹 · `/context` 看上下文状态 · `/portfolio` 看持仓 · `/plan` 看当前任务计划 · `/journal` 看日记 · `/memory` · `/sources` · `/cost` 看用量 · `exit` 退出（退出时自动整理记忆）。
...
| `engine.build_tool_schemas(...)` | 工具注册表（`src/tools.ts`） |
...
| `context_manager.compact_messages` | 上下文压缩 / 会话续接裁剪 |
| `start_allocation` + `playbooks/`（`tools/playbook.py`） | 技能按需加载（`src/skills/` + `SkillTool`） |
| `engine.run_turn` 循环（`engine.py`） | 主对话循环（`src/query.ts` / `QueryEngine.ts`） |
| `tool_catalog.py` + `/tools` | 工具注册表/能力自描述 |
| `trace_state.py` + `/trace` | 本轮工具执行轨迹 |
| `agent_profile.py` + `/agent`/`/doctor` | 运行态能力画像与本地体检 |
...
**机制**：`tools/memory.py` 提供 `save_memory/recall_memory/recall_memories/forget_memory`，把用户的
风险画像/持仓/偏好/决策写成 `agent_fin/memory/<分类>__<键>.md`；`main.py` 启动时调
`load_memory_block()` 注入精简记忆摘要；每轮再用当前问题检索 `load_relevant_memory_block()`，
只把相关记忆额外塞进本轮上下文。按 `分类+键` upsert，所以"纠正旧信息"就是再存一次（自动覆盖）。

**上下文管理**：`context_manager.py` 会在会话历史过长时，把较早对话压缩成「历史上下文摘要」，
保留最近回合与当前问题，并避免从孤立的 tool 结果开始续接。`/context` 可查看当前会话条数、
字符量、压缩次数和工具错误次数。

**工具调用护栏**：`engine._execute_tool` 对每次本地工具调用做超时、输出截断和结构化记录；
并在执行前按工具 schema 做必填/选填参数检查和简单类型规整。footer 会显示工具错误、截断次数与上下文
压缩情况；`/trace` 能查看最近工具调用，敏感参数会遮盖。可用 `FIN_TOOL_TIMEOUT_SECONDS` 和
`FIN_TOOL_OUTPUT_CHARS` 调整默认阈值。
删除持仓、删除记忆等高风险工具会在命令行向用户二次确认；如果没有确认通道或用户拒绝，工具不会执行。
模型文本默认流式输出，体感更接近 Claude Code；如遇兼容端点流式异常，可临时设 `FIN_STREAMING=0` 退回整段输出。
...
```
agent_fin/
├── engine.py        # 自写 agent loop：模型→tool_use→tool_result→再问
├── context_manager.py # 上下文统计/压缩，防止长会话撑爆窗口
├── agent_profile.py # 能力画像/运行体检（/agent、/doctor、agent_self_check）
├── tool_catalog.py  # 工具目录渲染（/tools）
├── trace_state.py   # 本轮工具调用轨迹（/trace）
├── main.py          # 入口：API 接入/状态 + 记忆/上下文装配 + REPL 循环
...
└── tools/
    ...
    ├── base.py      # 本地 @tool 装饰器
    ├── agent_status.py # agent_self_check：模型可调用的能力/体检工具
...
`/agent` 会输出它与 Claude Code 同构的能力映射；`/doctor` 只做本地检查，不调用网络。
