...
`/help` · `/tools` 看工具目录 · `/trace` 看工具轨迹 · `/context` 看上下文状态 · `/portfolio` 看持仓 · `/plan` 看当前任务计划 · `/journal` 看日记 · `/memory` · `/sources` · `/cost` 看用量 · `exit` 退出（退出时自动整理记忆）。
...
**30 个工具，分 9 组**：金融工作流(start_financial_workflow/final_task_check) · 显式计划(write_plan) ·
持仓与看板 · 风险诊断与调仓清单 · 投资日记与复盘 · 记忆 · 知识库 · 数据与量化。
实时资讯(get_news) · 报告导出/桌面通知(export_report/push_notification)。
