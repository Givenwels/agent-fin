...
试试：
  ...
  · 帮我做一套稳健型的资产配置
命令：/help · /agent · /doctor · /api · /tools · /trace · /context · /portfolio · /journal · /memory · /sources · /cost · exit 退出
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""

HELP_TEXT = """可用命令（本地直接执行，不耗 token）：
  /help  /agent 看能力画像  /doctor 运行体检  /api 看接口状态  /tools 看工具目录
  /trace 看工具轨迹  /context 看上下文状态
...
async def handle_local_command(user: str, stats: dict) -> bool:
    ...
    if cmd in ("/api", "/provider", "/model"):
        ...
        return True
    if cmd in ("/agent", "/capabilities", "/ability", "/abilities"):
        profile = agent_profile.build_agent_profile(
            tools=ALL_TOOLS,
            messages=stats.get("messages", []),
            stats=stats,
        )
        print(agent_profile.render_agent_profile(profile))
        return True
    if cmd in ("/doctor", "/status", "/health", "/check"):
        report = agent_profile.build_agent_doctor(tools=ALL_TOOLS, stats=stats)
        print(agent_profile.render_agent_doctor(report))
        return True
