...
query.ts），直接用 anthropic SDK 打到配置端点，自己驱动
...
async def ask(client, system: str, messages: list[dict], stats: dict) -> None:
    ...
    usage = await engine.run_turn(client, system, messages, on_text, on_tool,
                                  allow_delegate=True)
    dur = time.time() - t0
...
试试：
  ...
  · 帮我做一套稳健型的资产配置
命令：/help · /api · /portfolio · /journal · /memory · /sources · /cost · exit 退出
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""

HELP_TEXT = """可用命令（本地直接执行，不耗 token）：
  /help  /api 看接口状态  /portfolio 看持仓  /plan 看任务计划  /journal 看日记  /memory  /sources
